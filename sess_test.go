...
func listenEcho(port int) (net.Listener, error) {
	...
	//block, _ := NewAESBlockCrypt(pass)
	block, _ := NewSalsa20BlockCrypt(pass)
	return ListenWithOptions(fmt.Sprintf("127.0.0.1:%v", port), block, 10, 0, 0)
}
...
func TestListenerClose(t *testing.T) {
	port := int(atomic.AddUint32(&baseport, 1))
	l, err := ListenWithOptions(fmt.Sprintf("127.0.0.1:%v", port), nil, 10, 3, 0)
	if err != nil {
		t.Fail()
	}
...
func TestListenerOwnedPacketConn(t *testing.T) {
	...
	l, err := ListenWithOptions("127.0.0.1:0", nil, 0, 0, 0)
	if err != nil {
		panic(err)
	}
	defer l.Close()
...
func TestUDPSessionOwnedPacketConn(t *testing.T) {
	...
	client, err := DialWithOptions(l.Addr().String(), nil, 0, 0, 0)
	if err != nil {
		panic(err)
	}
	defer client.Close()
...
func TestUDPSessionNonOwnedPacketConn(t *testing.T) {
	...
	client, err := NewConn2(l.Addr(), nil, 0, 0, 0, pconn)
	if err != nil {
		panic(err)
	}
	defer client.Close()
